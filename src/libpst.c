...
int pst_open(pst_file *pf, const char *name) {
    ...
    _getcwd(pf->cwd, PATH_MAX+1);
    pf->fname = _strdup(name);
    return 0;
}
...
int  pst_reopen(pst_file *pf) {
    char cwd[PATH_MAX];
    if (!_getcwd(cwd, PATH_MAX))            return -1;
...
static pst_id2_tree* deep_copy(pst_id2_tree *head)
{
	...
	me = (pst_id2_tree*) pst_malloc(sizeof(pst_id2_tree));
    me->id2 = head->id2;
    me->id  = head->id;
            ...
            switch (list->elements[x]->mapi_id) {
                    ...
                    if (list->elements[x]->extra) {
                        if (list->elements[x]->type == 0x0101e) {
                            ...
                            for (i = 1; i <= array_element_count; i++) {
                                ...
                                ef->field_name = _strdup(list->elements[x]->extra);
                ...
                case 0x001A: // PR_MESSAGE_CLASS IPM.x
                    if ((list->elements[x]->type == 0x1e) ||
                        ...
                        if (!item->ascii_type) item->ascii_type = _strdup("unknown");
...
void pst_convert_utf8(pst_item *item, pst_string *str) {
    ...
    charset = pst_default_charset(item, sizeof(buffer), buffer);
    if (!strcasecmp("utf-8", charset)) return;  // already utf8
    DEBUG_ENT("pst_convert_utf8");
    newer = pst_vballoc(2);
    rc = pst_vb_8bit2utf8(newer, str->str, strlen(str->str) + 1, charset);
    if (rc == (size_t)-1) {
        free(newer->b);
...
pst_recurrence* pst_convert_recurrence(pst_item_appointment* appt)
{
    ...
    s = appt->recurrence_data.size;
    i = 0;
    p = appt->recurrence_data.data;
    if (p) {
